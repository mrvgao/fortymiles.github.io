...
(function(f, n, p, e, q) {
    ...
    g.About = function(x) {
        ...
        }, ["MathJax"]], " v" + MathJax.version, ["br"], s(u.replace(/ /g, ""), "using " + u), ["br"], ["br"], ["span", {
            style: {
                display: "inline-block",
                "text-align": "left",
                "font-size": "80%",
    ...
    f.Register.StartupHook("End Config", function() {
        ...
        }, c.RADIO("HTML-CSS", "renderer", {
            action: g.Renderer
        }), c.RADIO("Common HTML", "renderer", {
            action: g.Renderer,
            value: "CommonHTML"
        }), c.RADIO("Fast HTML", "renderer", {
            action: g.Renderer,
            value: "PreviewHTML"
        }), c.RADIO("MathML", "renderer", {
            action: g.Renderer,
            value: "NativeMML"
        }), c.RADIO("SVG", "renderer", {
            action: g.Renderer
        }), c.RADIO("PlainSource", "renderer", {
            action: g.Renderer,
            value: "PlainSource"
        }), c.RULE(), c.CHECKBOX("Fast Preview", "FastPreview"), c.CHECKBOX("Assistive MathML", "assistiveMML", {
        ...
        }, c.RADIO("MathJax", "context"), c.RADIO(["Browser", "Browser"], "context")), c.COMMAND(["Scale", "Scale All Math ..."], g.Scale), c.RULE().With({
        ...
        InternalMath: function(u, l) {
            ...
            if (u.match(/\\?[${}\\]|\\\(|\\(eq)?ref\s*\{/)) {
                while (q < u.length) {
                    ...
                    if (t === "$") {
                        if (r === "$" && o === 0) {
                            m.push(h.TeXAtom(d.Parse(u.slice(p, q - 1), {}).mml().With(n)));
                            r = "";
                            p = q
                        } else {
                            if (r === "") {
                                ...
                                if (t === "\\") {
                                    if (r === "" && u.substr(q).match(/^(eq)?ref\s*\{/)) {
                                        ...
                                        if (t === "(" && r === "") {
                                            ...
                                            if (t === ")" && r === ")" && o === 0) {
                                                m.push(h.TeXAtom(d.Parse(u.slice(p, q - 2), {}).mml().With(n)));
                                                r = "";
                                                p = q
                                            } else {
                                                if (t.match(/[${}\\]/) && r === "") {
        ...
        h.munderover.Augment({
            toPreviewHTML: function(q) {
                ...
                if (!n.displaystyle && this.data[this.base] != null && this.data[this.base].CoreMO().Get("movablelimits")) {
                    q = h.msubsup.prototype.toPreviewHTML.call(this, q);
                ...
                var p = this.PHTMLbboxFor(this.over),
                    s = this.PHTMLbboxFor(this.under),
                ...
                if (this.data[this.over]) {
                    ...
                    this.data[this.over].PHTMLhandleScriptlevel(t.firstChild.firstChild);
                    if (r != null) {
                        if (p.vec) {
                ...
                if (this.data[this.under]) {
                    ...
                    this.data[this.under].PHTMLhandleScriptlevel(q.lastChild)
                }
